...
# Sub_ Class of Bank called Customer

class Customer(Bank):
    def __init__(self,BankId, Bankname, Location, CustomerId, AccountNo, Name, Address, PhoneNo):
        # Super function to overide initialization in Base class
...
# Method to delete the details in dictionary

    def CloseAccount(self):
        del self.accounts[self.CustomerId]
# Method to allow application for loan by testing the required appropiate condition
...
class Teller(Customer, Bank):
    ...
    def LoanRequest(self, loanId, Type):
        self.loanType = Type
        self.loanId = loanId
        # Exception Handling For to allow only customers with with account only and and those with less amount in
        # account
        try:
            self.accounts[self.CustomerId][self.AcctNo]['accountNumber'] = self.AcctNo
        except:
...
# CODE TO RUN THE ABOVE CLASSES
# input from user
n = input('Enter Bank Id: ')
m = input('Enter Bank Name: ')
i = input('Enter Bank Location: ')
...
if d == 'C':
    ...
    while j < 11:
        j += 1
        A, B, C, D = input('Enter Customer Name:\n'), input('Enter your Address:\n'), \
        ...
        while True:
            E = int(input('Enter: 1 for General inquiry , 2 for Depositing , 3 for withdrawing, 4 for Requesting,'
